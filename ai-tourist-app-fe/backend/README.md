...
src/
  ...
  migrations/     forward-only SQL (001_init: users, api_keys, tourist_spots,
                  groups, group_members, location_pings + GIST index)
  shared/         middleware (JWT auth, zod validate, error handler), errors,
                  logger, types, asyncHandler
  auth/           POST /auth/signup | /login | /refresh  (bcrypt + JWT)
  keyvault/       POST/GET/DELETE /me/keys  (ciphertext blob; NEVER decrypt)
  spots/          placeholder (Sprint 2)
  group/          placeholder (Sprint 2)
  location/       placeholder (Sprint 2/3 — WS hub)
  notification/   placeholder (Sprint 3 — FCM/APNs)
...
is required. Run `npm test`. All 43 tests pass; `npm run lint` and
