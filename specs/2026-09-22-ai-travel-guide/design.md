# Design — AI Travel Guide (React Web)
...
**Key principle:** The backend is a **coordination layer**, not an AI gateway. The React client holds the decrypted API key in memory (decrypted from local AES-256 vault) and calls the user's AI provider **directly**. Backend only stores the encrypted key blob for cross-device restore and validates key shape on save. This satisfies REQ-AUTH-4 / NFR-SEC-1.

**Deployment:** Frontend deployed to Vercel (CDN edge, auto-scaling). Backend deployed separately (Railway, Render, or similar Node.js hosting).
...
| Geofence breach | LocationBroadcast evaluates 10km → Notification svc → Web Push | Server push |
...
### 3.1 React Client Modules

| Module | Responsibility | Key Packages |
|--------|---------------|--------------|
| `auth` | Login/signup, JWT storage, social login, session refresh | `axios`, `@react-oauth/google` |
| `key_vault` | Local AES-256 encrypt/decrypt of AI key, validation call to provider | `crypto-js`, localStorage |
| `location` | Geolocation API, background tracking (visibility API) | Native browser Geolocation API |
| `geofence` | Register geofences around tourist spots, detect entry, emit events | Custom geofence logic |
| `voiceover` | Build prompt, call AI provider (user key), parse response, TTS, playback, transcript UI | `axios`, Web Speech API or AI TTS |
| `map` | Render map, user marker, member markers, tourist-spot markers | `@googlemaps/js-api-loader` or `mapbox-gl` ⚠️ |
| `group` | Create/join/leave group, invite flow, member list, threshold config | `axios`, WS client |
| `walkie_talkie` | PTT button, WebRTC peer connection to SFU, floor indicator | `simple-peer` or native WebRTC |
| `settings` | API key entry, threshold, logout, preferences | — |
| `realtime` | WS client for location pings + member position updates | `socket.io-client` |
| `notification` | Web Push subscription, handle incoming geofence-breach push | `web-push` |
...
| `notification-service` | Consume `GeofenceBreachDetected` events; send Web Push | `web-push` |
...
-- (see §8 Security — recommended: client-side KEK in localStorage with encryption).
...
|---|----------|---------|--------|-----------|
| D1 | Map SDK | Google Maps / Mapbox / OSM | **Google Maps JavaScript API** ⚠️ | Best integration with React, familiar, generous free tier for MVP. Mapbox if usage exceeds free quota. OSM (`leaflet`) if license-cost-sensitive. **Trade-off:** Google = cost at scale + Google account dependency. |
...
| D3 | TTS source | Provider TTS (OpenAI audio) / platform-native (Web Speech API) | **Provider TTS with native fallback** ⚠️ | Provider TTS = high quality, billed to user's key (consistent BYOK). Native = free, offline-capable, lower quality. Let user toggle in settings; default to provider. |
...
| D5 | Social login | Google / Apple / both | **Google only** ⚠️ | Apple Sign-In is primarily for native apps. For web, Google OAuth is the standard choice. |
...
| D8 | Location strategy | Continuous high-freq / geolocation polling | **Geolocation API + visibility-based polling** | Battery-conscious approach using browser's native Geolocation API with configurable accuracy. |
...
| D10 | Frontend deployment | Vercel | **Vercel** | Edge CDN, auto-scaling, excellent DX, free tier sufficient for MVP. |
...
| Layer | Tool | Scope |
|-------|------|-------|
| **React unit** | Vitest + React Testing Library | Component logic: geofence math, prompt builder, key encryption/decryption, threshold comparison, floor-state machine |
| **React component** | Vitest + React Testing Library | Home screen, group list, settings, PTT button states, transcript view |
| **React E2E** | Playwright | End-to-end on browser: login → see map → mock geofence trigger → voiceover plays; group join → member marker appears |
| **Backend unit** | Jest | Service logic: JWT, key-vault blob handling, geofence breach calc, group invariants, event handlers |
| **Backend integration** | Jest + Supertest + Testcontainers | REST endpoints, WS event flow, pub/sub fan-out |
| **Contract** | OpenAPI schema + WS schema validation | Client/server agree on shapes |
| **E2E (system)** | Playwright against a staging backend | Full user journey |

> **Note:** For React E2E, Playwright is the ideal choice (it targets web browsers natively). Flutter E2E is NOT applicable since we're using React web.

**TDD ordering:** domain/pure-logic unit tests first (geofence math, floor state, encryption) → component tests → integration tests. Backend: service unit → API integration → contract.
...
1. On first API-key entry, client generates a random 256-bit KEK, stores it in `localStorage` (encrypted with a user-derived key from password/PBKDF2).
...
6. Cross-device restore: user re-enters credentials, pulls the blob, decrypts.
...
- Certificate pinning optional for web (limited support).
- HSTS on backend.
...
- Geolocation API requires explicit user permission with clear in-app rationale.
...
|---|----------|------------------|-----------|
| 1 | Map SDK | **Google Maps JavaScript API** ⚠️ | Best React integration, solid free tier. Switch to Mapbox if cost/usage demands. |
...
| 4 | Social login providers | **Google only** ⚠️ | Web app context; Google OAuth is the standard choice. |
...
- **React state:** Zustand or React Context for global state; local state for component-specific concerns.
- **Offline (NFR-REL-1):** service worker for caching, IndexedDB for offline voiceover data. AI inference inherently needs network — degrade by showing cached transcript + "offline" banner.
- **PWA:** Use Workbox for service worker generation, manifest.json for installability.
- **SFU choice:** mediasoup (lower-level, more control, Node.js-friendly) vs LiveKit (batteries-included, has official JS SDK). **Recommend LiveKit for MVP** (faster integration, official client) — revisit if custom floor control needs mediasoup. ⚠️
- **PostGIS:** essential for `ST_DWithin` nearest-spot and breach-distance queries without pulling all rows.
- **Vercel:** frontend deploys via `vercel` CLI or GitHub integration. Environment variables for API URL, map API key.
